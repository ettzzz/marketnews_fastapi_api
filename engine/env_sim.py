...
class simEnvironment:
    ...
    def init_weight(self, codes):
        ...
        ### when doing his_operator.insert_weight_data there is a non-zero mapping inside
...
if __name__ == "__main__":
    ...
    # conn.execute("DELETE FROM news_weight;")
    # his_operator.off(conn)
    ...
    for date in date_ranger:
        ...
        for i in range(len(DAILY_TICKS) - 1):
            ...
            news = his_operator._get_news(
                source=source,
                date=date,
                start_timestamp=timestamper(
                    date + " " + start_time, "%Y-%m-%d %H:%M:%S"
                ),
            ...
            if end_time[-1] == "0" and is_open_day:
                his_operator.insert_weight_data(
                    sim_env.weights_dict, date + " " + end_time
                )
            if end_time[-1] == "9":
